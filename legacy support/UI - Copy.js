       ...
       const sheet = document.getElementById('moveHistory');
        ...
        if (fenInputBox) {
            fenInputBox.addEventListener('input', (e) => {
                ...
                this.#game.engine.setGameMode(currentMode);
                const validation = this.#game.engine.validate_fen(newFen);
                
                if (validation.valid) {
                    // 2. Lock the new position logically (resets rootNode/currentNode and PGN headers)
                    this.#game.loadNewPosition(newFen, currentMode);
...
resizeApp() {
        ...
        try {
            if (game) {
                ...
                if (gMode === 'crazyhouse' || gMode === 'bughouse' || gMode === 'placement') {
                    // ✨ DYNAMIC POCKET FIX: Only trigger the wide pocket layout 
                    // if renderPockets() has actually populated the containers with captured pieces!
       ...
       if (bottomPanel) {
            ...
            if (isStudy) { 
                ...
                // ✨ THE FIX: Safely measure the pocket and eval bar directly here!
                ...
                // ✨ Update chart width to match the newly expanded target width
                let exactWidth = lW + boardW + pW + safePocketW + safeEvalW;
                
                if (lW > 0) exactWidth += 40; 
                if (pW > 0) exactWidth += 40; 
                
                // If the pocket is open, accommodate the 80px gap we added!
...
updatePuzzleStats() {
        ...
        // 2. Update Score
        if (scoreEl) {
            scoreEl.innerText = this.#game.puzzleScore || 0;
        }
...
renderPockets(pocket) {
        ...
        // ✨ THE FIX: Force the flexbox into a single centered column!
        if (topPocket) {
            topPocket.innerHTML = '';
            topPocket.style.setProperty('flex-direction', 'column', 'important');
            topPocket.style.setProperty('flex-wrap', 'nowrap', 'important');
            topPocket.style.setProperty('align-items', 'center', 'important');
            topPocket.style.setProperty('gap', '8px', 'important');
        }
        if (bottomPocket) {
            bottomPocket.innerHTML = '';
            bottomPocket.style.setProperty('flex-direction', 'column', 'important');
            bottomPocket.style.setProperty('flex-wrap', 'nowrap', 'important');
            bottomPocket.style.setProperty('align-items', 'center', 'important');
        ...
        const drawPocket = (container, color) => {
            ...
            pieceChars.forEach(pChar => {
                ...
                if (pieceCounts[pType]) {
                    ...
                    // ✨ Slightly larger size (60px) since they have a whole column to themselves now!
...
switchTab(tabName) {
        ...
        if (this.#game) {
            ...
            if (this.#game.mode === 'editor' && lowerTab !== 'editor') {
                ...
                if (currentFen && this.#game.engine && !this.#game.engine.validate_fen(currentFen).valid) {
        ...
        if (engineBtn) {
            ...
            // Safe puzzle check
            if (isPuzzle && this.#game && !state.isGameOver && state.puzzle && state.puzzle.active) {
...
toggleEditorMode(active) {
        try {
            ...
            if (!this.#game.isPlayingLiveGame) {
                this.#game.mode = active ? 'editor' : 'analysis';
            }
...
initEditorBars() {
        ...
        if (topBar) {
            topBar.innerHTML = `<div class="tool-group">
                ${['P','N','B','R','Q','K'].map(p => `
                    <div class="tool-btn" onmousedown="this.startSpareDrag(event,'b','${p}')">
                        ${getSafeImgHtml('b', p)}
                    </div>`).join('')}
            </div><div class="tool-btn trash-btn" onclick="this.setEditorTool('trash', this)">${trashIcon}</div>`;
        }
        ...
        if (bottomBar) {
            bottomBar.innerHTML = `<div class="tool-group">
                ${['P','N','B','R','Q','K'].map(p => `
                    <div class="tool-btn" onmousedown="this.startSpareDrag(event,'w','${p}')">
                        ${getSafeImgHtml('w', p)}
                    </div>`).join('')}
            </div><div class="tool-btn trash-btn" onclick="this.setEditorTool('trash', this)">${trashIcon}</div>`;
        }
...
renderBoard(animate = false, showMangaTail = true, overrideMove = null) {
            ...
            if (state.mode === 'editor') {
                ...
                sq.onmousedown = (e) => {
                    ...
                    if (this.editorTool === 'trash') {
                        ...
                        this.#game.editBoard(i, { color: color, type: type });
                        this.renderBoard(false);
                    }
                };
            }
        ...
        piecesMap.forEach((p, id) => {
            ...
            if (activeMove && p.idx === activeMove.to && this.#game && this.#game.currentNode && this.#game.currentNode.nag) {
                const info = this.getNagInfo(this.#game.currentNode.nag);
        ...
        this.renderArrows();
        if(document.getElementById('fenDisplay')) document.getElementById('fenDisplay').innerText = this.#game.currentNode.fen;
        const resignBtn = document.getElementById('resignBtn');
        if (resignBtn) {
        ...
        if (this.#game && this.#game.engine && typeof this.#game.engine.pocket === 'function') {
            if (typeof this.renderPockets === 'function') {
                this.renderPockets(this.#game.engine.pocket());
            }
        }
...
renderHistoryImmediate() {
        ...
        if (this.#game && this.#game.rootNode) {
            currentTreeSize = this.getTreeSize(this.#game.rootNode);
        }
        ...
        if (this.pgnStyle === 'tree') {
            ...
            if (this.#game && this.#game.rootNode) {
                this.renderTreeVertical(this.#game.rootNode, list);
            }
        } else {
            list.className = 'history-list pgn-standard';
            if (this.#game && this.#game.rootNode) {
                this.renderTreeRecursive(this.#game.rootNode, list, 1);
            }
        }
...
createPlyDiv(node) {
        ...
        d.onclick = (e) => {
            ...
            if (this.#game.goToNodeId(targetNodeId)) {
                const freshState = this.#game.getReader();
                this.renderBoard(false); 
                this.updateHistory(); this.renderArrows();
                if (freshState.mode !== 'play' && this.#game.updateStockfish) this.#game.updateStockfish();
            }
        };
...
updateEvalBar(type, val) {
        ...
        let vWinner = null;
        if (typeof this.#game.engine.variant_winner === 'function') {
            vWinner = this.#game.engine.variant_winner();
        }
        ...
        const isDraw = this.#game.engine.in_draw() || 
                       this.#game.engine.in_stalemate() || 
                       (typeof this.#game.engine.in_threefold_repetition === 'function' && this.#game.engine.in_threefold_repetition());
...
showAnnotationPopup(e, node) {
        ...
        if (hasComment) {
            actionsContainer.appendChild(createActionBtn('🗑️', 'Delete Comment', () => {
                this.#game.updateComment(node.id, "");
                forceRedraw(); 
            }));
        }
        ...
        if (node.parent) {
            actionsContainer.appendChild(createActionBtn('⬆️', 'Promote Variation', () => {
                if (this.#game) this.#game.promoteVariation(node.id);
                forceRedraw(); 
            }));
            
            actionsContainer.appendChild(createActionBtn('🌟', 'Make Main Line', () => {
                if (this.#game) this.#game.makeMainline(node.id);
                forceRedraw(); 
            }));

            actionsContainer.appendChild(createActionBtn('❌', 'Delete from here', () => {
                if (this.#game) this.#game.deleteNode(node.id);
                forceRedraw();
            }, true)); 
        }
            ...
            for (let i = 0; i < displayMoves.length; i++) { 
                ...
                if (moveObj) {
                    ...
                    span.onmouseenter = (e) => {
                        ...
                        if (this && this.hoverEngineMove) this.hoverEngineMove(fenAtMove, e, duckSq);
                    };
                    span.onmouseleave = () => {
                        span.style.color = '';
                        span.style.textDecoration = 'none';
                        if (this && this.stopHoverEngineMove) this.stopHoverEngineMove();
                    };
                    span.onclick = (e) => {
                        e.stopPropagation();
                        if (this.#game && this.#game.playEngineSequence) {
                            this.#game.playEngineSequence(seqString, currentFen);
                        }
                    };
...
getPly(node) {
let c = 0;
if (this.#game.pgnHeaders &&this.#game.pgnHeaders['FEN']) {
...
updateEditorInputs() {
document.getElementById('fenInput').value = this.#game.generateFEN();
}
updateEditorState() {
this.#game.turn = document.getElementById('editorTurn').value;
this.#game.castling.wK = document.getElementById('castling-wK').checked;
this.#game.castling.wQ = document.getElementById('castling-wQ').checked;
this.#game.castling.bK = document.getElementById('castling-bK').checked;
this.#game.castling.bQ = document.getElementById('castling-bQ').checked;
...
processTrashAction(e) {
        ...
        if (e.type === 'mousedown' || (e.type === 'mousemove' && e.buttons === 1)) {
            ...
            if (this.#game.board[idx] !== null) {
                this.#game.board[idx] = null; 
                
                // 1. Sync internal engine to visual board
                this.#game.syncEngineToBoard(); 
                
                // 2. Generate the new FEN and update the game tree
                const newFen = this.#game.generateFEN();
                this.#game.pgnHeaders = { "FEN": newFen, "SetUp": "1" };
                if (this.#game.rootNode) this.#game.rootNode.fen = newFen;
...
editorReset() {
        ...
        this.animateToStartPosition(startFen, () => {
            ...
            if (this.#game) this.#game.lastMove = null; 
            this.selectedSq = null;
            this.legalMoves = [];
            ...
            if (this.#game && typeof this.#game.syncEngineToBoard === 'function') {
                this.#game.syncEngineToBoard();
            }
...
finishEditor() {
        ...
        this.#game.gameOver = true; // Keep it frozen until the user selects an opponent
...
updatePieceImagesSafe(pieceSet) {
        ...
        pieces.forEach(pieceEl => {
            ...
            if (this.#game && this.#game.board) {
                pieceObj = this.#game.board.find(p => p && p.id == pieceId);
            }
...
copyFEN() {
navigator.clipboard.writeText(this.#game.currentNode.fen);
...
updatePlayerInfo() {
        ...
        // 2. Handle Human vs Human / Local Mode
            ...
            this.playerInfo['w'].name = (this.#game && this.#game.pgnHeaders['White']) || "White";
            this.playerInfo['b'].name = (this.#game && this.#game.pgnHeaders['Black']) || "Black";
            ...
            this.avatars['w'] = (this.#game && this.#game.isEngineMatch) || isEngine(wName) ? engineImg : humanImg;
            this.avatars['b'] = (this.#game && this.#game.isEngineMatch) || isEngine(bName) ? engineImg : humanImg;
        }
...
updateClocks() {
        ...
        if (this.#game && this.#game.isPlayingLiveGame) {
            wTime = this.#game.whiteTime; 
            bTime = this.#game.blackTime;
            ...
            if (this.#game && this.#game.pgnHeaders && this.#game.pgnHeaders['TimeControl']) {
                const parts = this.#game.pgnHeaders['TimeControl'].split('+');
        ...
        if (this.#game && this.#game.isPlayingLiveGame) {
            const activeTurn = this.#game.currentLiveTurn || this.#game.turn;
...
showPromotionModal(color, destIdx, callback) {
        ...
        let pieces = ['q', 'n', 'r', 'b']; 
        if (this.#game && this.#game.gameMode === 'antichess') {
            pieces.push('k');
        }
...
jumpToChartMove(idx) {
        ...
        if (curr) {
            if (!curr.id) curr.id = 'n_' + Math.random().toString(36).substr(2, 9);
            const success = this.#game.goToNodeId(curr.id);
            if (success) {
                const state = this.#game.getReader();
                this.renderBoard(false);
                this.updateHistory();
                this.renderArrows();
                if (this.updateClocks) this.updateClocks();
                if (state.mode !== 'play' && this.#game.updateStockfish) {
                    this.#game.updateStockfish();
                }
            }
        }
...
jumpToNextError(color, type) {
        ...
        this.#game.currentNode = targetNode;
        this.#game.loadFEN(targetNode.fen);
        this.#game.goToNodeId(targetNode.id);
        this.renderBoard(false);
        this.updateHistory();
        this.renderArrows();
        if (this.updateClocks) this.updateClocks();
        if (this.#game.updateStockfish && !this.#game.isPlayingLiveGame) {
            this.#game.updateStockfish();
        }
...
updateChartActiveLine() {
        ...
        while (curr && curr.children.length > 0) {
            curr = curr.children[curr.selectedChildIndex || 0];
            ply++;
            if (curr === this.#game.currentNode) activeIdx = ply;
        }
...
openChapterModal(idx = -1) {
        ...
        if (idx === -1) {
            ...
            nameInput.value = `Chapter ${this.#game.chapters.length + 1}`;
            orientInput.value = 'w';
            ...
            //  EDIT CHAPTER MODE
            const chap = this.#game.chapters[idx];
            ...
            delBtn.style.display = this.#game.chapters.length > 1 ? "block" : "none";
        }
...
switchChapterTab(tabName) {
        ...
        if (tabName === 'empty') {
            ...
            dataInput.value = this.#game ? this.#game.generateFEN() : "";
            ...
            if (extractedGames.length > 0) {
                ...
                setTimeout(() => {
                    if (extractedGames.length > 1) {
                        ...
                        this.#game.allStudies.push(newStudy);
                        this.#game.loadStudy(newStudyId);
                        ...
                        const chapterMatch = gameStr.match(/\[ChapterName\s+"([^"]+)"\]/);
                        const eventMatch = gameStr.match(/\[Event\s+"([^"]+)"\]/);
                        const title = chapterMatch ? chapterMatch[1] : (eventMatch ? eventMatch[1] : `Chapter ${this.#game.chapters.length + 1}`);
                        
                        this.#game.chapters.push({ title: title, pgn: gameStr.trim(), analysisMode: 'Normal analysis' });
                        this.#game.loadChapter(this.#game.chapters.length - 1);
                    }
...
quickImport() {
        ...
        this.switchTab('analysis'); 
            ...
            if (text.includes('[Event') || text.includes('1.')) this.#game.loadPGN(text);
            else this.#game.loadNewPosition(text);
...
openChapterManager() {
        ...
        this.#game.chapters.forEach((ch, idx) => {
            ...
            text.innerText = `${idx + 1}. ${ch.title}`;
            text.style.flex = "1";
            text.style.color = idx === this.#game.activeChapterIndex ? "#38bdf8" : "#fff";
            text.style.fontWeight = idx === this.#game.activeChapterIndex ? "bold" : "normal";
...
renderStudyList() {
        ...
        studies.forEach((study, idx) => {
            ...
            title.style.flex = "1";
            title.style.fontWeight = study.id === this.#game.currentStudyId ? "bold" : "normal";
            title.style.color = study.id === this.#game.currentStudyId ? "#38bdf8" : "#fff";
...
generateGIF() {
        ...
        const originalNode = this.#game.currentNode;
        ...
        this.#game.currentNode = this.#game.rootNode;
        this.#game.engine.load(this.#game.rootNode.fen);
        ...
        const captureFrameLoop = async () => {
            ...
            // Move Forward!
            if (this.#game.currentNode.children && this.#game.currentNode.children.length > 0) {
                ...
                this.#game.currentNode = originalNode;
                this.#game.engine.load(originalNode.fen);
                this.renderBoard(false);

...
#processEngineComment(node, rawComment) {
        ...
        if (finishedLiveGame && typeof this.#saveState === 'function') {
            ...
            this.mode = 'analysis';     // Briefly pretend we are in analysis mode
            this.#saveState('analysis'); // Save the completed PGN to the analysis cache
            this.mode = originalMode;   // Revert back so the modal shows up properly
        }
...
#puzzleSuccess() {
        ...
        if (isRush) {
            this.puzzleScore++;
            if (typeof window.ui !=='undefined') window.ui.updatePuzzleStats();
            setTimeout(() => {
            ...
            setTimeout(() => {
                this.mode ='analysis'; 
                this.gameOver = false;
                if (typeof window.ui !=='undefined') window.ui.updateHistory();
            }, 50);
        }
    }
#puzzleFail() {
...
toggleArrow(from, to, color) {
        ...
        if (typeof window !== 'undefined' && window.ui) {
            if (typeof window.ui.refreshLiveDot === 'function') window.ui.refreshLiveDot(this.currentNode);
...
updateComment(nodeId, text) {
        ...
        if (node) {
            ...
            if (typeof window !== 'undefined' && window.ui && typeof window.ui.updateHistory === 'function') window.ui.updateHistory();
        }
    }
...
stepForward() {
        if (this.currentNode && this.currentNode.children.length > 0) {
            ...
            if (typeof window.ui !== 'undefined') {
                ...
                // Forward move ALWAYS animates and shows tail
...
stepBack() {
        if (this.currentNode.parent) {
            ...
            if (typeof window.ui !== 'undefined') {
                ...
                if (visualMove) this.triggerMoveSound(visualMove);
                
                // Backward move ALWAYS animates and shows tail!
                window.ui.renderBoard(true, true, visualMove);
...
updateSettingsTime() {
        ...
        if (!this.isPlayingLiveGame || this.currentNode === this.rootNode) {
            ...
            if (typeof window.ui !== 'undefined') window.ui.updateClocks(); 
        }
    }
...
loadNewPosition(fen, explicitMode = null) {
        ...
        if (!validation.valid) {
            if (window.ui) window.ui.showNotification("Invalid FEN for " + targetMode + ": " + validation.error, "Error", "⚠️");
            return; 
        }
...
parseArrowsAndCircles(node, comment) {
        ...
        if (typeof window.ui !== 'undefined' && window.ui.renderArrows) {
            window.ui.renderArrows();
        }
    }
loadPGN(pgn, isEditor = false, isInternalLoad = false) {
        ...
        if (window.ui) {
            ['updateHistory', 'renderBoard', 'renderArrows', 'renderHeaders', 'highlightLastMove', 'updateClocks', 'updateStatus', 'scrollToActiveMove', 'showNotification', 'updatePuzzleStats', 'updatePlayerNames', 'displayMetadata','renderCharts'].forEach(m => silence(window.ui, m, backups.ui));
        }
        ...
        try {
            ...
            this.isPaused = false; 
            this.pgn = "";
            if (window.ui && window.ui.togglePgnEditing) window.ui.togglePgnEditing(true);
            if (window.ui && window.ui.toggleReviewButton) window.ui.toggleReviewButton(true);
            ...
            if (window.ui) {
                Object.keys(backups.ui).forEach(m => window.ui[m] = backups.ui[m]);
                try {
                    if (this.currentNode) {
                        ...
                        if (window.ui.updatePgnAvatars) window.ui.updatePgnAvatars(this.pgnHeaders['White'], this.pgnHeaders['Black'], this.isEngineMatch, true);
...
attemptPremove() {
        ...
        if (!actualPiece || actualPiece.color !== move.color || actualPiece.type !== move.piece) {
            this.clearPremoves();
            if (typeof window.ui !== 'undefined') window.ui.renderBoard(true);
            return;
        }
    ...
    if (this.#engine && this.#engine.game_over()) return null;
        ...
        if (this.mode === 'study' && !isAutoReply && this.chapters && this.chapters[this.activeChapterIndex]) {
            ...
            if (currentMode === 'interactive' || currentMode === 'Interactive lesson') {
                ...
                if (!isCorrectMove) {
                    if (window.ui) {
                        window.ui.showNotification("Inaccuracy! Try finding a better move.", "Incorrect", "❌");
                        window.ui.renderBoard(false); // Snap the piece back
                    }
                    return null; // Cancel move execution
                } else {
                    if (window.ui) window.ui.showNotification("Good move!", "Correct", "✅");
                    ...
                    setTimeout(() => {
                        if (this.currentNode && this.currentNode.children.length > 0) {
                            ...
                            if (window.ui) window.ui.showNotification("Lesson Complete!", "Success", "🏆");
                        }
                    }, 500);
                }
            }
        ...
        if (this.mode === 'puzzle') {
            ...
            if (userStr !== solStr && !this.#engine.in_checkmate()) {
                ...
                if (window.ui) ui.renderBoard(false);
                this.#puzzleFail();
                return null; 
            }
...
const vol = (typeof window.ui !== 'undefined' && typeof window.ui.volume === 'number') ? window.ui.volume : 0.7;
